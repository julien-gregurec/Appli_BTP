...
> Ce document remplace intégralement la version précédente (datée du 12-13 juillet 2026), devenue obsolète : elle décrivait une « sortie de mode prototype » (accès `anon` étendu sur les tables métier, script `supabase/production/sortie_mode_prototype.sql`) qui **n'existe plus dans le schéma actuel**. Vérifié directement sur `elsatia-preview` le 07-08-2026 : 0 table publique avec RLS désactivé, seules 4 tables ont un grant `anon` — `catalogue_options_abonnement`, `catalogue_services_mise_en_service`, `modeles_roles_predefinis`, `plans_abonnement` — toutes en `SELECT` seul, sur des catalogues publics de tarification, sans aucune donnée métier. Le script `sortie_mode_prototype.sql` et les références aux migrations « 30-37 »/« 43-44 » de l'ancienne version sont sans objet aujourd'hui (193 fichiers de migration, dernière `20260806000199`, architecture RLS complètement différente) ; ils ne doivent plus être suivis. **Mise à jour 07-08-2026** : ce script a été confirmé cassé et dangereusement obsolète par l'audit de préparation Production (référence une fonction supprimée, liste blanche de permissions RPC périmée d'environ 105 fonctions) — archivé sous `supabase/production/archive/NE_PAS_EXECUTER_sortie_mode_prototype.sql`, à ne jamais exécuter. Les scripts de recette restants dans `supabase/production/` (créer/nettoyer des entreprises de démonstration) sont désormais protégés par un garde-fou programmatique — voir `supabase/production/README.md`.
>
