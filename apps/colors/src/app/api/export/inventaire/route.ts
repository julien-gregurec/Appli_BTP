...
export async function GET() {
  ...
  const { data, error } = await supabase.from("colors_seaux")
    .select("marque,produit,reference_produit,teinte_nom,teinte_reference,couleur_hex,mode_quantite,quantite_nominale,quantite_restante,unite,pourcentage_restant,etat,created_at,updated_at,colors_emplacements(nom)")
  ...
  const entetes = ["Marque","Produit","Référence","Teinte","Référence teinte","HEX","Mode","Quantité nominale","Quantité restante","Unité","Pourcentage restant","État","Emplacement","Ajouté le","Mise à jour"];
  const lignes = (data ?? []).map((s) => [s.marque,s.produit,s.reference_produit,s.teinte_nom,s.teinte_reference,s.couleur_hex,s.mode_quantite,s.quantite_nominale,s.quantite_restante,s.unite,s.pourcentage_restant,s.etat,(s.colors_emplacements as {nom?:string}|null)?.nom,s.created_at,s.updated_at].map(csv).join(";"));
