-- Rollback of 20260920030000_studio_export_profiles. Append-only usage data and 720p jobs are never destroyed.
begin;
do $$begin
 if exists(select 1 from public.studio_usage_events) then
  raise exception 'Usage events exist; rollback refused to preserve the ledger';
 end if;
 if exists(select 1 from public.studio_render_jobs where profile='hd720') then
  raise exception '720p render jobs exist; rollback refused to preserve them';
 end if;
end$$;
drop trigger studio_record_export_usage on public.studio_render_outputs;
drop function public.studio_record_export_usage();
drop function public.studio_workspace_usage(uuid,timestamptz);
drop table public.studio_usage_events;
alter table public.studio_render_jobs drop constraint studio_render_jobs_profile_check;
alter table public.studio_render_jobs add constraint studio_render_jobs_profile_check check(profile in ('preview','standard'));
...
delete from supabase_migrations.schema_migrations where version='20260920030000';
commit;
