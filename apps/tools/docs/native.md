...
|---|---|---|
| Web/PWA | hébergement Web | `sw-tools.js`, cache `elsatia-tools-v4` |
...
Les clés restent `elsatia.tools.*`. Au premier démarrage natif, les données déjà disponibles dans le stockage Web de la WebView sont importées sans écraser une valeur native existante. Preferences convient aux favoris et récents ; une future volumétrie de projets justifierait une couche SQLite séparée.
...
Le listener Capacitor est prêt et n’accepte que `/` et `/outils/<slug>` sur `https://tools.elsatia.fr`. L’activation Store est différée car elle exige :
...
Les capacités seront exposées derrière de petits ports TypeScript avec un adaptateur Web et un adaptateur Capacitor : partage, fichiers, export PDF, caméra, haptique, orientation, notifications et capteurs. Le moteur métier ne devra jamais importer directement un plugin natif.
...
Le partage natif du résultat est reporté : la valeur ajoutée ne compense pas encore l’interface et les tests supplémentaires. L’architecture permet d’ajouter `@capacitor/share` sans toucher aux calculs.
...
iOS contient `PrivacyInfo.xcprivacy` pour l’usage de UserDefaults par Capacitor Preferences (`CA92.1`). Aucune permission caméra, localisation, microphone ou notification n’est déclarée. Toute future capacité devra ajouter sa permission et mettre à jour les déclarations de confidentialité au même moment.
