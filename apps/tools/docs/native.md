...
`NEXT_PUBLIC_TOOLS_ENV` n’est plus seulement descriptive : elle décide de la sévérité de la garde `scripts/verify-public-env.mjs`, exécutée avant `next build` et `build:native`. En `native-production` — comme en `production`, et comme lorsque la variable est absente — l’absence de `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` ou `NEXT_PUBLIC_TOOLS_BILLING_API_URL` interrompt le build. L’URL d’abonnement est requise sur le build natif au même titre que sur le Web : `monetization-client.ts` l’appelle pour vérifier côté serveur chaque achat Apple ou Google. Un export natif de développement se déclare `NEXT_PUBLIC_TOOLS_ENV=native-dev`. Le détail figure dans le [README](../README.md#variables-publiques-et-garde-de-build).
