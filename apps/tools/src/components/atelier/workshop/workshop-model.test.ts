...
import { EMPTY_FREE_GEOMETRY, addFreeEntity, createFreeEntity } from "../../../lib/tracing/free-geometry";
...
describe("modes d'affichage (§12)", () => {
  ...
  it("le mode Forme ne montre QUE le résultat fini", () => {
    ...
    expect(layers).toEqual({
      ...
      points: false,
      labels: false,
      // WORKSHOP-UI-CANONICAL-V2 §11 — le tracé libre est le relevé de l'artisan lui-même,
      // jamais un décor : aucun mode ne l'éteint, sans quoi la moitié des projets du canon
      // (mode `free`) s'ouvrirait sur un plan vide.
      free: true,
    });
  });

  it("aucun mode n'éteint le tracé libre", () => {
    for (const mode of WORKSHOP_MODES) expect(layersForMode(mode.id).free).toBe(true);
  });
...
/* ---- Réconciliation avec le canon (WORKSHOP-UI-CANONICAL-V2) ------------- */

describe("scène d'un tracé LIBRE (§5/§11)", () => {
  /** Géométrie libre réelle, projetée par le canon — aucune fixture de scène. */
  function freeSource() {
    const geometry = addFreeEntity(
      EMPTY_FREE_GEOMETRY,
      createFreeEntity(
        "segment",
        [
    ...
    const withPoint = addFreeEntity(geometry, createFreeEntity("point", [{ x: 500, y: 500 }], "pt-1"));
    return freeGeometryToShape(withPoint, { id: "libre", name: "Relevé", frame: "sheet" });
  }

  it("montre le tracé de l'artisan dans le mode par défaut", () => {
    const scene = workshopFreeScene(freeSource(), createWorkshopState("forme"));
    expect(scene.segments).toHaveLength(1);
  });

  it("n'invente ni axes, ni construction, ni cotes sur un tracé libre", () => {
    const scene = workshopFreeScene(freeSource(), createWorkshopState("cotations"));
    expect(scene.constructionLines).toEqual([]);
    expect(scene.dimensions).toEqual([]);
  });

  it("éteindre le calque « Tracé libre » vide la scène sans toucher au tracé", () => {
    const source = freeSource();
    const state = toggleWorkshopLayer(createWorkshopState("construction"), "free");
    const scene = workshopFreeScene(source, state);
    expect(countSceneEntities(scene)).toBe(0);
    // Le tracé source est intact : rallumer le calque le remontre entier.
    expect(countSceneEntities(workshopFreeScene(source, createWorkshopState("construction")))).toBeGreaterThan(0);
  });

  it("ne recadre jamais le plan en masquant un calque", () => {
    const source = freeSource();
    const hidden = workshopFreeScene(source, toggleWorkshopLayer(createWorkshopState("forme"), "free"));
    expect(hidden.bounds).toEqual(source.bounds);
  });
});

describe("calques réellement alimentés (§11)", () => {
  it("ne propose le tracé libre qu'aux tracés libres, et l'inverse", () => {
    expect(PARAMETRIC_LAYERS).not.toContain("free");
    expect(FREE_LAYERS).toContain("free");
    // Un tracé libre ne publie ni axes, ni construction, ni cotes de modèle : les proposer
    // donnerait trois interrupteurs qui ne masquent rien (§22 — aucun bouton mort).
    for (const dead of ["axes", "construction", "dimensions"] as const) {
      expect(FREE_LAYERS).not.toContain(dead);
    }
  });

  it("chaque calque proposé existe bien dans le registre des calques", () => {
    const known = new Set(WORKSHOP_LAYERS.map((layer) => layer.id));
    for (const layer of [...PARAMETRIC_LAYERS, ...FREE_LAYERS]) expect(known.has(layer)).toBe(true);
  });
});

describe("source affichée par l'écran (§5)", () => {
  it("un tracé libre reçoit le plan de tracé libre", () => {
    expect(workshopSource("free", false)).toBe("free");
    // Même avec un modelId résiduel : `tracingProjectMode` a déjà tranché en faveur du libre.
    expect(workshopSource("free", true)).toBe("free");
  });

  it("un tracé paramétrique reçoit le plan du modèle", () => {
    expect(workshopSource("parametric", true)).toBe("parametric");
  });

  it("un projet encore vide reçoit le plan de tracé libre, jamais un écran mort", () => {
    // C'est l'état d'un projet qui vient de choisir « dessin libre » dans l'assistant :
    // `tracingProjectMode` le dit `undecided` tant qu'aucune primitive n'existe, et attendre
    // `free` interdirait justement de poser la première.
    expect(workshopSource("undecided", false)).toBe("free");
  });

  it("un modèle inconnu du registre reste paramétrique, pour que l'erreur se voie", () => {
    // `hasModel` vrai, descripteur introuvable : c'est un projet à réparer. Le basculer en
    // libre masquerait le message du résolveur derrière un plan vide.
    expect(workshopSource("parametric", true)).toBe("parametric");
  });
});
