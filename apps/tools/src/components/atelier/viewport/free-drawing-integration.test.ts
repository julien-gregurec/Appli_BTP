...
function openFreeAtelier() {
  ...
  const state = {
    ...
    select(pixel: { x: number; y: number }, additive = false) {
      ...
      // ATELIER-FREE-DRAWING-R2-RECONCILIATION-V1 — la règle de sélection R2 est reproduite
      // ici telle que `AtelierViewportWorkspace` l'applique, et non via un helper parallèle :
      // Shift + clic à vide PRÉSERVE la sélection, un clic simple à vide la vide.
