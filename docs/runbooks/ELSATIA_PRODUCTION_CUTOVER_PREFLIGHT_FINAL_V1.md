...
> **Rebasage 2026-09-05 — cible cutover figée sur `36642e3` / ledger 263.** Deux lots validés
> depuis la version 1.2 (`36642e3` remplace l'ancien SHA cible `c1930ab` partout dans ce
...
- **Preuves d'exécution offline au SHA `36642e3` (Fresh 263, Restore→263, rollback, drift ACL)** :
...
| SHA cible | `ac7bf050…` | **`36642e3fa9c894ce0d77fd3926de03d2240a9741`** |
...
# → colonne "Remote" = ledger réel ; comparer au fichier supabase/migrations/ de 36642e3
...
sentinelles. Le diff `versions canoniques (36642e3) − versions Production` **est** la liste
d'application du §2.
...
Cible canonique : **263 migrations** (`supabase/migrations/*.sql` @ `36642e3`,
...
(`git diff fcdd4e7c..36642e3 -- supabase/migrations/` : aucune migration existante touchée).
...
La liste exacte = **toute version de `supabase/migrations/` @ `36642e3` absente du ledger
...
- `verify:migrations` (dans un environnement portant le code `36642e3`) = 263 uniques ;
...
2. **Un cutover coordonné DB + app est nécessaire.** Ordre : migrations (T0) → déploiement de
   l'app sur **`36642e3`** (T+15) → contrôles → ouverture (T+30). L'app `36642e3` est la seule
...
3. **Point de non-retour exact : application de `20260902000255_acl_reconciliation_v1`.**
   ...
   mais elles élargissent le schéma que seul `36642e3` sait exploiter.
...
1. **Déployer l'app canonique (`36642e3`)** sur la Production Branch correcte (**pas `main`**).
...
- **Contrat multi-app** (compte partagé, catalogue d'accès, redirect URLs Colors/Tools) : porté
  ...
  le modifie pas.
- **Devient immédiatement fonctionnel après migrations + déploiement `36642e3`** :
  ...
  - R3 — section **Modules** de `/abonnement` (inclus plan / ajoutés / disponibles), garde
    ...
    `/abonnement/module-non-inclus` (correctif boucle post-login `36642e3`) ;
...
- [ ] Calculer le diff `versions(36642e3) − versions(Production)` → **liste d'application ordonnée** (§2).
- [ ] Vérifier SHA cible `36642e3` et SHA Production actuel (`fcdd4e7c` attendu).
...
- [ ] Déployer l'app sur **`36642e3`** (Production Branch correcte, **pas `main`**).
...
| P0-2 | ~~Répétition Fresh 263 + Restore 263 + pgTAP + rollback `210 → 263 → 210`~~ | **FERMÉ 2026-09-04** — drill offline complet au SHA `36642e3`, preuves §13.1–§13.3 | — |
...
| P0-4 | ~~Diff ACL applicatif Fresh↔Restore rejoué au SHA `36642e3`~~ | **FERMÉ 2026-09-04** — `application_acl_drift = 0` (6 294 lignes normalisées, 0 diff), preuve §13.4 | — |
...
- Pile : projet Supabase local `btp-platform` (existant), `supabase db reset --local` au SHA
  `36642e3`+docs (`6be33d4`, sans effet sur les migrations).
...
### 13.5 Tests finaux (SHA `36642e3` + docs `6be33d4`)
...
```text
P0-2 (Fresh/Restore/rollback au SHA 36642e3) : FERMÉ
...
| **T+30** | Déploiement app `36642e3` (§19) démarré + premiers contrôles (login, absence 5xx critique, absence boucle de redirection, `/abonnement`, dashboard, chantier, stock, module inclus/non inclus). | — |
...
migrations `supabase/migrations/*.sql` @ `36642e3` (aucune version canonique manquante de la
...
- [ ] SHA app `36642e3fa9c894ce0d77fd3926de03d2240a9741` disponible et prêt à déployer (build
      Vercel préparé)
...
Déployer **uniquement** `36642e3fa9c894ce0d77fd3926de03d2240a9741` sur la Production Branch
correcte.
...
- [ ] Aucune boucle de redirection (le correctif `36642e3` couvre le cas compte dépôt sans
