...
-- rejeu des 321 migrations de la ref de préparation (ELSATIA_PREVIEW_EXECUTION_PREP_V3).
...
controles(ordre, controle, attendu, observe, ok, bloquant) as (
  select 1, 'migrations appliquées (registre CLI)', '321, dernière 20260922000330',
         coalesce(m.nb::text || ', dernière ' || m.derniere, 'registre absent (harnais local)'),
         m.nb is null or (m.nb = 321 and m.derniere = '20260922000330'), true
  from migrations m
  union all
