...
## V1.3 — fermeture DML directe et consultation persistante de la file

La migration append-only `20260828000249_colors_security_cleanup_v13.sql` complète `00246`, `00247` et `00248` sans modifier aucune migration existante ni centrale. Elle s'applique aussi bien après le ledger Colors complet qu'après le socle canonique `24c944da09664d9aa1523af6e047f9b84437b97e` suivi de `00246 → 00247 → 00248`.

- **Écriture directe fermée** : `revoke insert, update, delete, truncate, references, trigger` sur toutes les tables `public.colors_*` pour `PUBLIC`, `anon` et `service_role`. Le journal `colors_mouvements` est en plus fermé en écriture pour `authenticated` (il n'était concédé qu'en `SELECT` par `00246`, mais certains socles ré-accordent INSERT/UPDATE par privilèges par défaut). `service_role` contourne la RLS : la protection ne repose donc plus sur la seule RLS ni sur les seuls triggers. Les écritures légitimes passent exclusivement par les RPC `SECURITY DEFINER` (propriétaire `postgres`, non membre d'aucun rôle API, aucune chaîne `SET ROLE` depuis `authenticator`). Les droits `EXECUTE` des fonctions `colors_*` sont resserrés à `authenticated`.
- **Consultation persistante et cloisonnée** : `colors_nettoyages_photos_seau(uuid)` — `STABLE`, `SECURITY DEFINER`, `search_path` fixé, aucun DML. Filtrage entièrement serveur : entreprise déduite du seau, habilitation vérifiée par `colors_action_autorisee(...,'voir')`. Ne retourne que l'identifiant du seau, l'état, `nettoyage_requis`, le nombre de tentatives, les horodatages et l'erreur technique assainie ; jamais le chemin Storage interne ni la file d'un autre tenant. Exécutable par `authenticated` uniquement.
- **Persistance après rechargement** : la fiche seau (`inventaire/[id]`) consulte cette surface au chargement serveur et affiche un bloc `Nettoyage photo en attente` tant qu'une dette `a_nettoyer` existe. Le signal ne dépend plus de l'état mémoire de la réponse d'upload ; il disparaît uniquement lorsque la tâche est réellement résolue (`colors_resoudre_nettoyage_photo` après disparition effective de l'objet Storage).
- **Idempotence de la file** : `unique(photo_path)` + `on conflict do update` (00248) garantit une seule dette active par suppression, des tentatives ultérieures tracées et une résolution idempotente ; aucun effacement d'historique, aucune suppression de la nouvelle photo.
- **Tests indépendants du SQLSTATE** : les refus de sécurité des suites `colors_functional_core_v1`, `colors_integrity_v11` et `colors_correctifs_v12` sont désormais vérifiés par leur comportement observable (opération refusée + absence effective de mutation : lignes, contenu, historique, tâches de nettoyage) plutôt que par un `42501` précis — un même refus est stoppé par privilège sur un socle et par trigger métier sur un autre. Les refus portés par un message métier stable de l'application sont vérifiés via `throws_like`. Les violations de contrainte `CHECK` (SQLSTATE `23514`, invariant) restent vérifiées telles quelles.
- **Préflight** : `scripts/security/colors-acl-preflight.mjs` (lecture seule) échoue si un DML direct est accordé à `anon`/`service_role` sur une table `colors_*`, si une fonction `colors_*` `SECURITY DEFINER` n'a pas de `search_path`, si une fonction `colors_*` est exécutable par `anon`/`service_role`, si une fonction Colors contient du SQL dynamique, ou si la RPC de consultation est absente / trop largement exécutable.

Validation locale du lot :

- ledger Colors : **217 migrations** (`00246 → 00247 → 00248 → 00249`), suite pgTAP complète de **33 fichiers / 566 tests** verts ;
- ledger canonique depuis `24c944da09664d9aa1523af6e047f9b84437b97e` : **228 migrations** (`00245 → 00246 → 00247 → 00248 → 00249`), les **5 fichiers pgTAP Colors (168 tests)** verts ;
- préflight ACL Colors : conforme sur les deux ledgers ;
- Colors : **61 tests Vitest**, TypeScript et ESLint réussis ;
- racine : **529 tests Vitest**, TypeScript réussi, ESLint sans erreur (trois avertissements `<img>` préexistants hors Colors) ;
- vérification des migrations : 217 noms/horodatages uniques ; scan des secrets : aucun secret ; `npm audit --audit-level=high` racine et Colors : zéro vulnérabilité ; `git diff --check` propre.

Réserve reportée, hors périmètre V1.3 : sur la **reconstruction locale du ledger canonique**, quatre tests **centraux** (`platform_aal2_role_integrity_v1`, `platform_discount_column_guard_r71`, `platform_support_isolation_audit_v1`) échouent déjà **sans** `00249` — ils supposent un durcissement des privilèges des tables `plateforme_*` que le socle `24c944d` ne porte pas dans une reconstruction locale nue. `00249` ne modifie aucune de ces tables et n'ajoute **aucun** nouvel échec ; corriger ce point relève du lot d'intégration canonique (migrations centrales, interdites en V1.3).

Aucun déploiement, aucune Preview, aucune migration distante, aucun appel OCR réel, aucune modification Stripe/F4 ni de compte n'ont été réalisés.
