...
**Si la demande est arrivée par le fil in-app** (`/plateforme/support`) plutôt que par `support@elsatia.fr` : répondre dans le fil notifie automatiquement le demandeur par e-mail, avec un **extrait de 240 caractères** de la réponse (voir `docs/operations/MATRICE_EMAILS_V1.md` §« réponse du support »). Ne jamais faire figurer dans une réponse support une donnée personnelle d'un tiers, un élément de pièce d'identité ou le détail d'un périmètre de suppression : ce début de réponse sort de l'application par e-mail.
