...
| Compte sup. Administratif/Chef équipe/Terrain | — | — | — | `STRIPE_PRICE_COMPTE_SUP_*` | **Absent en Production** (présent en Preview seulement) — voir §10 et constat P0 ci-dessous |
...
| Comptes supplémentaires (3 types) | À CRÉER EN LIVE | À CRÉER EN LIVE | À CRÉER EN LIVE | `STRIPE_PRICE_COMPTE_SUP_*` | À CRÉER EN LIVE (et manquant même en Test Production, cf. constat ci-dessous) |

> **Constat hors périmètre Live, mais actif dès aujourd'hui** : `reconcilierAbonnementStripe` (`src/lib/stripe-abonnement.ts:297-320`) tourne quotidiennement en Production (cron `15 3 * * *`, `src/app/api/cron/abonnements/route.ts`) et à chaque activation de compte employé. Faute des 6 variables `STRIPE_PRICE_COMPTE_SUP_*` en Production, elle échoue silencieusement (`prix_supplement_absent`) sans jamais facturer les comptes ajoutés au-delà du quota inclus (`comptesInclus`), lequel n'est lui-même vérifié nulle part en amont. **Aucune limite n'est donc appliquée à la création de comptes, et aucun dépassement n'est facturé, y compris en Test.** Documenté ici pour mémoire ; un lot correctif dédié est nécessaire (hors périmètre P15, qui est audit/préparation uniquement).

## 8. Séparation Preview / Production, Test / Live

- **Preview** (`elsatia-preview`, Vercel) : `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_ABONNEMENT_SECRET` et les 10 `STRIPE_PRICE_*` (offres) **+ 6 `STRIPE_PRICE_COMPTE_SUP_*`** — toutes en environnement Vercel "Preview" uniquement, toutes Test. Webhook dédié `we_1Tziay0...` pointant vers l'URL de déploiement Preview.
- **Production** (`elsatia-production`, Vercel) : `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_ABONNEMENT_SECRET`, 10 `STRIPE_PRICE_*` — en environnement "Production" uniquement, actuellement Test (confirmé par le smoke test 22-08-2026 : Checkout affichant "Environnement de test"). **Manque les 6 `STRIPE_PRICE_COMPTE_SUP_*`** (cf. §7).
