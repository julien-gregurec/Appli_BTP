...
| Recette Isolation Multi-Entreprises — vérification préalable | **PHASE 0 VALIDÉE** (5 août 2026) : projet Supabase `elsatia-preview` (réf. `pgvvpqyjziyapbbkydmc`, région `eu-west-3`/Paris, UE) confirmé ; variable Vercel `SUPABASE_SERVICE_ROLE_KEY` confirmée présente, scopée Preview uniquement ; correspondance clé ↔ projet confirmée manuellement par l'utilisateur (jamais de valeur secrète affichée, copiée ni enregistrée) | Le provisionnement effectif de l'entreprise fictive B (écriture réelle) reste une action distincte nécessitant sa propre autorisation explicite avant la Phase 1 (tests d'isolation A↔B) |
