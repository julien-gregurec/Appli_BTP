...
Version 2026-09-14 (session autonome, mise à jour isolation RC). Issu de la recette preview
...
minimal nécessaire à GP V1. Rapport complet : « GP V1 — Isolation Release Candidate » (2026-09-14).
`feat/gp-v1-metier-devis-planning-references-v1` reste intacte comme référence de comportement validé, mais
n'est plus la source de la mise en Production.

Périmètre : ledger `20260710000001` → `20260914000296` (238 fichiers, dont l'extraction ciblée
`20260911000281_gp_v1_rc_acl_prerequisites.sql` qui remplace, pour ce périmètre, les REVOKE de la migration
`20260902000255_acl_reconciliation_v1` — voir § 5.2), branche **`release/gp-v1-rc`** (HEAD
`6741dd7927187f36ef0084edb4747485469703ee` au 2026-09-14), projet Supabase Production
...
2. `release/gp-v1-rc` poussée sur origin, **non fusionnée** dans `release/commercialisation-v1` (c'est
   ...
   voir § 6) ; `npm run verify:migrations` (238 valides), `npm run verify:secrets`, `git diff --check` verts ;
   `npx tsc --noEmit`, `npm run lint`, vitest complet (1147/1151 — 3 skip, 1 échec confirmé environnemental
   sur `xlsx.test.ts`, passant isolément), pgTAP complet (565/565 sur le Fresh, 17 suites) verts.
...
- Versions en attente = fichiers locaux (`release/gp-v1-rc`) ∖ ledger distant ; `db push --linked --dry-run
  --include-all` doit lister exactement ces versions (les numéros inférieurs au maximum distant — 200, 232,
  236→240 — exigent `--include-all`). Aucune version distante absente en local (sinon STOP : ledger inconnu).
- **Absence attendue et volontaire** : `20260902000255_acl_reconciliation_v1.sql` n'est PAS dans
  ...
  extrait dans `20260911000281_gp_v1_rc_acl_prerequisites.sql` (321 fonctions, 762 tables, 6 séquences). Ce
...
2. Appliquer les versions en attente dans l'ordre de `db push`. Le résultat attendu, prouvé le
   2026-09-14 sur conteneur Docker jetable (checkpoints `gprc-baseline-211` → `gprc-rc-full`) : les 27
   migrations retenues (210-215, 257, 265, 272, 274, 275, 281, 282-296) s'appliquent sans erreur sur la
   211-baseline de `release/commercialisation-v1`.
...
4. pgTAP sur le clone migré (17 suites GP + isolation multi-tenant) : **565/565 assertions vertes**, prouvé
   le 2026-09-14 (conteneur `gprc-pgtap`). Fixture `isolation_multitenant.inc` adaptée pour ce périmètre
   (insert `plateforme_admins` réduit à `email, role` — colonnes réellement présentes) ; adaptation appliquée
   uniquement à la copie de test dans le conteneur, jamais au fichier du dépôt.
...
2. Contrôles immédiats : `count(*)` et `max(version)` de `supabase_migrations.schema_migrations` = 238 /
...
3. **Contrôle spécifique à ce périmètre (281 puis 296)** : `recalc_totaux_devis(uuid)` NON exécutable par
   `authenticated` ni `service_role` (REVOKE de la 281) ; aucune erreur `permission denied for function
...
| Répétition générale rouge (clone de la sauvegarde → 296, pgTAP) | § 4 | STOP |
...
| Toute erreur `57014` (statement timeout) à l'enregistrement d'un devis | journaux | STOP — régression des 292/293 (renumérotées 282/293 dans ce périmètre), analyser avant d'annoncer |
