-- ADMIN-GLOBAL-V1-R7.2 : sépare l'orchestrateur service_role de l'autorité
-- d'observation Stripe. PostgreSQL ne possède que des clés publiques Ed25519 et
-- ne peut jamais émettre une attestation acceptable.

create extension if not exists pgsodium;

create schema if not exists stripe_attestation;
revoke all on schema stripe_attestation from public,anon,authenticated,service_role;

-- Ces deux tables sont volontairement absentes des schémas PostgREST exposés.
-- Leur provisionnement est une opération DBA : la migration reste fail-closed
-- tant qu'un environnement et une clé publique n'ont pas été installés.
create table stripe_attestation.configuration (
  singleton boolean primary key default true check (singleton),
  environment text not null check (environment in ('test','live'))
);

create table stripe_attestation.public_keys (
  key_id text primary key check (key_id ~ '^[a-z0-9_.:-]{1,64}$'),
  environment text not null check (environment in ('test','live')),
...
create table stripe_attestation.consumed_attestations (
  jti uuid primary key,
  operation_id uuid not null unique references public.plateforme_operations_remise(id) on delete restrict,
  key_id text not null references stripe_attestation.public_keys(key_id) on delete restrict,
  signature bytea not null check (octet_length(signature)=64),
  consumed_at timestamptz not null default now()
);

revoke all on all tables in schema stripe_attestation from public,anon,authenticated,service_role;

alter table public.plateforme_operations_remise
  add column preuve_attestation_payload jsonb,
  add column preuve_attestation_signature text,
  add column preuve_attestation_jti uuid,
  add column preuve_attestation_key_id text,
  add column preuve_attestation_environment text,
  add column preuve_attestation_action text,
  add column preuve_attestation_expire_at timestamptz;

create unique index plateforme_operations_remise_attestation_jti_idx
  on public.plateforme_operations_remise(preuve_attestation_jti)
  where preuve_attestation_jti is not null;
...
create function stripe_attestation.canonical_payload(p_payload jsonb)
...
create function public.plateforme_finaliser_operation_remise_attestee_serveur(
  p_operation_id uuid,
  p_verrou_token uuid,
  p_attestation jsonb,
  p_signature text
...
begin
  ...
  if p_attestation->>'version'<>'1' then raise exception 'Version d''attestation Stripe invalide'; end if;
  ...
  elsif p_attestation->>'coupon_id' is not null
  ...
  update public.plateforme_operations_remise set
    ...
    etat_observe_apres_stripe=jsonb_build_object('coupon_id',p_attestation->>'coupon_id'),
    ...
    preuve_etat_observe=jsonb_build_object('coupon_id',p_attestation->>'coupon_id'),
...
do $$
begin
  if current_setting('r72.had_f4_membership')='false' then
