...
--
-- Provenance : lot qualifié séparément sur la branche `fix/commande-reception-
-- stock-v1` (SHA `2a2034cc9a4036a325961aa1dc62fa9e1fcea0d8`), verdict
-- `COMMANDE STOCK FLOW QUALIFIED`, rapport
-- `docs/qualification/ELSATIA_GP_COMMANDE_RECEPTION_STOCK_V1.md`. Cette
-- branche partait du train `e0a83ebbd430efb5217466f05be3c08f45ae4de5`, où le
-- numéro `20260922000318` était encore libre. Porté ici tel quel (même
-- architecture, même contrat, même logique — voir le rapport ci-dessus pour
-- le raisonnement complet) sur le train `claude/compassionate-euler-5j6avr`
-- (HEAD `38cf99e50d53c0c0dc6e80871c0a178addf6e9f1` au moment de l'intégration),
-- où `20260922000318` est déjà pris par
-- `20260922000318_dashboard_indicateurs_bornes.sql` (lot Performance/
-- Dashboard, intégré entretemps) : seul le numéro change, en `322`, premier
-- horodatage réellement libre après `20260922000321` sur ce HEAD (vérifié :
-- `ls supabase/migrations | sort | tail` s'arrête à 321, aucune migration
-- 322+ n'existe). Aucun chevauchement fonctionnel constaté avec les
-- migrations 299–321 du train (aucune ne touche
-- lignes_commande/commandes_fournisseurs/mouvements_stock/receptions
-- côté commande fournisseur ; seule 317, sans rapport, corrige
-- next_reference()). Le test pgTAP qualifié est porté à l'identique dans
-- `supabase/tests/gp_reception_commande_stock_transactionnel_v1.test.sql`
-- (65 assertions, inchangées).
--
