-- Corrige public.ajouter_audit_note_frais, cassée depuis sa création
-- (20260713000060) : digest() n'est jamais résolu car pgcrypto est installé
-- dans le schéma extensions (`create extension if not exists "pgcrypto";`,
-- sans clause SCHEMA — convention Supabase par défaut) alors que la fonction,
-- security definer, verrouille son search_path sur public seul
-- (`set search_path=public`). Résultat vérifié sur le SQL actuel de cette
-- branche : tout appel échoue avec "function digest(text, unknown) does not
-- exist" — et cette fonction est le tout premier appel de la chaîne d'audit
-- des notes de frais (journal_audit_notes_frais), donc toute création de
-- note de frais échoue avant même la création du brouillon.
--
-- Corrigé par qualification explicite du schéma (extensions.digest), sans
-- élargir le search_path implicite de la fonction — même correctif minimal
-- que la source, aucun autre changement de comportement.
--
-- Porté depuis integration/gp-external-pilot-closure-v1 (commit 3b25041,
-- migration source
-- 20260819000217_terrain_mobile_v1b_fix_digest_search_path.sql).
