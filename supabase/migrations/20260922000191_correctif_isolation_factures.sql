...
-- (factures_chantier_entreprise_fkey, migration 20260710000020).
--
-- Les policies RLS de public.factures ne contrôlent que factures.entreprise_id lui-même —
-- elles garantissent qu'un utilisateur écrit bien dans SA propre entreprise, mais ne
-- garantissent jamais que les ressources liées (client, devis, factures d'origine/parente)
-- appartiennent à cette même entreprise. La RLS protège l'appartenance de la ligne, pas
-- l'intégrité de ses relations : ce sont deux garanties distinctes, et seule une contrainte
-- structurelle (clé étrangère) peut garantir la seconde de façon fiable, y compris face à un
-- appel direct à l'API REST Supabase qui contournerait tout contrôle applicatif.
--
...
-- chantier_id. Les quatre colonnes concernées sont nullables (facture manuelle sans devis,
-- sans facture d'origine, sans facture parente — vérifié sur
-- supabase/migrations/20260710000006_factures.sql, 20260715000080_suite_metier_complete.sql
-- et 20260724000177_lien_avoir_facture_origine.sql) : une clé étrangère composite n'est
-- jamais vérifiée lorsque l'une de ses colonnes est NULL, donc ces cas restent pleinement
-- fonctionnels sans traitement particulier.
--
-- Aucune policy RLS n'est modifiée par cette migration. Les clés étrangères simples
-- existantes (client_id -> clients(id), etc.) sont volontairement conservées : cette
-- migration est additive.
--
-- Porté depuis integration/gp-external-pilot-closure-v1 (ancêtre du commit 8f5fca1, 379fd7b,
-- migration source 20260806000197_correctif_isolation_factures.sql).
