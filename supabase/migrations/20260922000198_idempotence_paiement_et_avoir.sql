-- Ferme deux trous d'idempotence côté métier (encaissement facture, création
-- d'avoir) : correctifs de correction, pas de stricte sécurité, mais réels et
-- vérifiés contre le code actuel de cette branche.
--
-- ─────────────────────────────────────────────────────────────
-- 1. ENCAISSEMENT (paiements) — course TOCTOU non protégée
-- ─────────────────────────────────────────────────────────────
--
-- enregistrerPaiementAction (src/app/actions/factures.ts) fait un simple
...
-- avant l'écriture (le trigger `recalc_paiements_facture` ne fait qu'un
-- constat après coup, il n'empêche rien). Correctif : verrou `for update` sur
-- `factures` avant de comparer le montant au reste dû, dans une RPC dédiée.
...
-- ─────────────────────────────────────────────────────────────
-- 2. AVOIR — verrou absent, aucune règle anti-doublon
-- ─────────────────────────────────────────────────────────────
--
-- creer_facture_avancee ne verrouille pas la ligne `devis` avant écriture, et
-- rien ne compare un nouvel avoir à ceux déjà émis pour la même facture
-- d'origine. Un double clic sur "Créer un avoir" insère donc deux avoirs
-- intégraux identiques contre la même facture, doublant le crédit accordé au
-- client. Correctif : verrou for update sur le devis + index unique partiel
-- + pré-check applicatif qui résout vers l'avoir existant (jamais une erreur
-- brute) + filet de sécurité sur unique_violation pour la vraie course.
