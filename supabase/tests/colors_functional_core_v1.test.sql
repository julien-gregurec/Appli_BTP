...
-- V1.4 : la définition de la photo principale (ligne « photo associée par
-- action métier ») laisse désormais elle aussi une trace. L'historique du seau
-- compte donc 6 événements : entree, consommation, deplacement, ouverture,
-- photo, archivage.
select is((select count(*) from public.colors_mouvements where seau_id='c0200000-0000-0000-0000-000000000001'),6::bigint,'historique conservé après archivage');
select is((select count(*) from public.colors_mouvements where seau_id='c0200000-0000-0000-0000-000000000001' and type='photo'),1::bigint,'changement de photo tracé (V1.4)');
