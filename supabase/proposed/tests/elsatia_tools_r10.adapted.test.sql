-- COPIE ADAPTÉE de supabase/tests/elsatia_tools_r10.test.sql pour la proposition tools_projects_server_side_access_v1
-- (deux blocs `ADAPTATION D2` ajoutés, rien d'autre). À substituer à l'original quand la migration sera numérotée.
begin;
...
-- ADAPTATION D2 : l'ancien test écrivait dans le cloud avec la seule habilitation `tools_pro`, sans entitlement Pro —
-- c'est l'abus fermé par tools_projects_server_side_access_v1. Le membre reçoit ici un entitlement Pro.
reset role;
insert into public.entitlements_utilisateurs_elsatia(utilisateur_id,application_code,niveau,capabilities,source) values('10000000-0000-0000-0000-000000000002','tools','pro',array['saved-projects'],'internal');
set local role authenticated;
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000002',true);
...
-- ADAPTATION D2 : l'entitlement est retiré pour retrouver le cas « entitlement absent ».
reset role;
delete from public.entitlements_utilisateurs_elsatia where utilisateur_id='10000000-0000-0000-0000-000000000002';
set local role authenticated;
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000002',true);
