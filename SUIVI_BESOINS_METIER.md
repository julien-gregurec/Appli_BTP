...
| Accès selon les postes | ✅ Fait | Les modules sans droit Consulter sont absents du menu/dashboard et leurs URLs redirigent. La migration 43 impose aussi les droits Gérer dans Supabase. La liste Employés indique pour chacun si l’accès est activé, son poste et ses droits Consulter/Gérer/Personnel, sous forme de cartes lisibles sur mobile et de tableau sur ordinateur. |
...
| Notes de frais salarié vers comptable | ✅ Fait | Photo/PDF privé, montant, catégorie et statut. Le salarié connecté ne peut déposer et consulter que ses propres notes ; le droit de gestion comptable est séparé pour valider, refuser ou rembourser. |
...
| Installation ou invitation collaborateur | 🟡 Prêt à activer | PWA installable depuis l’inscription, invitation personnelle BTP ou code entreprise, puis activer sa fiche, rejoindre ou créer une entreprise. L’usage réel des comptes attend la bascule volontaire hors prototype. |
...
| Utilisation complète sur mobile | ✅ Fait | Interface responsive et application installable sur l’écran d’accueil. Les listes Employés, Flotte, Outillage, Clients, Chantiers, Devis et Factures ont des cartes mobiles dédiées avec leurs informations métier et actions principales ; les filtres et tableaux détaillés restent sur ordinateur. Bouton d’installation Android/Chrome et guide Safari pour iPhone ; aucune donnée privée mise en cache. |
