...
## 13. Décisions effectives du Lot E Video Render Engine

`workers/studio-video` interprète le contrat D avec FFmpeg seul. Le Web autorise une demande ; la transaction SQL fige timeline/révision/assets et crée le job avec son outbox. BullMQ/Redis transporte uniquement l'UUID. Le worker séparé télécharge les originaux privés en streaming, compose des intermédiaires temporaires, encode un MP4 H.264/AAC puis publie une sortie privée. Aucune modification des tables, migrations historiques ou sémantiques A–D.

`studio_render_jobs`, `studio_render_outputs` et `studio_render_outbox` sont ajoutés par la migration 257. Lecture RLS par projet/workspace ; viewer lit et télécharge, editor/admin/owner lancent et annulent. Une lease avec heartbeat et vérification d'autorisation protège chaque progression/publication. Une sortie unique par job, chemin contenant le jeton de lease, aucune écriture directe des clients. Une réponse réseau perdue à la publication ne provoque pas la suppression d'un MP4 potentiellement déjà confirmé.

Les six transitions D occupent le début du clip entrant, avec l'image finale précédente figée. Les sept mouvements photo sont interpolés à partir du document. Les paramètres fixes D (rotation 0, scale 1, playback 1, centre de base) restent fixes ; des valeurs futures non qualifiées échouent explicitement. Les quatre résolutions standard sont 1080×1920, 1920×1080, 1080×1080 et 1080×1350, à 30 fps ; preview interne moitié résolution et dimensions paires. Les previews/downloads du bucket `studio-renders` sont signés pour 60 secondes après autorisation.

Concurrence 1, deadline configurable, limites mémoire/disque, cancellation et réconciliation manuelle locale. Aucun déploiement du worker, scheduler Production, Remotion, musique globale, IA ou Lot F. Détails d'exploitation et preuves : [contrat de rendu](ELSATIA-STUDIO-RENDER-CONTRACT.md), [rapport E](ELSATIA-STUDIO-V1-LOT-E-REPORT.md).
...
### Lot H — analyse locale et sélection explicite

L’analyse ajoute `studio_media_analysis` (cache et job versionné par asset) et un worker BullMQ séparé `studio-analysis-media-v1`. Le provider local mesure des frames réduites via OpenCV headless ; aucune dépendance externe IA. Le flag serveur `STUDIO_AI_ANALYSIS` est OFF par défaut. Les groupes restent dérivés par projet, car les assets sont partageables entre projets d’un même workspace. Le moteur D reçoit seulement une sélection validée et explicitement confirmée, crée une nouvelle version et respecte l’ordre fourni même pour un template chronologique. Les anciens parcours A–G restent disponibles sans analyse. Détails, seuils, données et limites : [contrat d’analyse H](ELSATIA-STUDIO-AI-ANALYSIS-CONTRACT.md).
