...
export async function modifierPolitiqueNotesFraisAction(formData: FormData) {
  ...
  if (error) retour(messageErreurUtilisateur("modifierPolitiqueArchivageAction", error, "Impossible d’enregistrer cette politique d’archivage."), true);
...
export async function ajouterCategorieNoteFraisAction(formData: FormData) {
  ...
  if(error) retour(error.code==="23505"?"Cette catégorie existe déjà":messageErreurUtilisateur("creerCategorieNoteFraisAction",error,"Impossible de créer cette catégorie."),true);
...
export async function changerCategorieNoteFraisAction(id:string,actif:boolean) {
  ...
  if(error) retour(messageErreurUtilisateur("changerCategorieNoteFraisAction",error,"Impossible de mettre à jour cette catégorie."),true);
...
export async function poserLegalHoldNoteFraisAction(noteId:string,formData:FormData) {
  ...
  if(error) redirect(`/notes-frais/${noteId}?error=${encodeURIComponent(messageErreurUtilisateur("poserLegalHoldNoteFraisAction",error,"Impossible de poser ce blocage légal."))}`);
...
export async function leverLegalHoldNoteFraisAction(noteId:string,holdId:string) {
  ...
  if(error) redirect(`/notes-frais/${noteId}?error=${encodeURIComponent(messageErreurUtilisateur("leverLegalHoldNoteFraisAction",error,"Impossible de lever ce blocage légal."))}`);
