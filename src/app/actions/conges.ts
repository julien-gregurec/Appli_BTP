...
export async function creerDemandeCongeAction(formData:FormData){
 ...
 if(error||!data?.id)retour(error?messageErreurUtilisateur("creerDemandeCongeAction:insert",error,"Impossible de créer la demande de congé."):"Création impossible",true);
 ...
 if(transitionError)retour(messageErreurUtilisateur("creerDemandeCongeAction:soumettre",transitionError,"Impossible de soumettre la demande de congé."),true);revalidatePath("/conges");retour("Demande envoyée au responsable");
}
export async function transitionDemandeCongeAction(id:string,action:string,formData?:FormData){if(isEmailLoginDisabled())retour("Compte personnel requis",true);const sb=await createClient();const {error}=await sb.rpc("transition_demande_conge",{p_demande_id:id,p_action:action,p_message:formData?txt(formData,"message")||null:null});if(error)retour(messageErreurUtilisateur("transitionDemandeCongeAction",error,"Impossible de mettre à jour cette demande de congé."),true);revalidatePath("/conges");revalidatePath("/planning");retour(action==="approuver"?"Congé approuvé et ajouté au planning":"Demande mise à jour");}
