...
export default async function NotesFraisPage({ searchParams }: { searchParams: Promise<{ error?: string; statut?: string; categorie?: string; chantier?: string; employe?: string }> }) {
  ...
  let requete = supabase.from("notes_frais").select("id,reference,date_frais,montant_ttc,devise,categorie,fournisseur,statut,statut_export,verrouille_at,employe:employes(id,prenom,nom),chantier:chantiers!notes_frais_chantier_entreprise_fkey(nom)")
