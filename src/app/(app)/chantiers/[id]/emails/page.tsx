...
export default async function EmailsChantierPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return <main className="p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    {connexions?.some((connexion) => connexion.statut === "active") && <aside className="rounded border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Synchronisation de messagerie</strong><p className="mt-1">Une boîte active peut classer automatiquement les messages reconnus par la référence du chantier.</p></aside>}
