    ...
    <main className="p-4 sm:p-8">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {peutVoirNotesEquipe&&<section className="space-y-4 rounded-md border border-neutral-200 p-4 dark:border-neutral-800">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Notes de frais du chantier</h2><p className="text-sm text-neutral-500">Dépenses des employés rattachées à ce chantier. Les montants en vérification ne sont pas comptabilisés dans les dépenses validées.</p></div><Link href={`/notes-frais?chantier=${id}`} className="rounded-md border px-3 py-2 text-sm font-medium">Voir ou ajouter une note de frais</Link></div>
          ...
          <div className="divide-y dark:divide-neutral-800">{(notesFrais??[]).map(note=>{const employe=relation(note.employe as {prenom:string;nom:string}|{prenom:string;nom:string}[]|null);const statut=statutNoteFrais(note.statut);return <Link key={note.id} href={`/notes-frais/${note.id}`} className="grid gap-1 py-3 text-sm hover:bg-neutral-50 dark:hover:bg-neutral-900 sm:grid-cols-[1fr_auto_auto] sm:items-center sm:gap-4"><div><strong>{note.reference}</strong><p className="text-xs text-neutral-500">{note.fournisseur??"Sans fournisseur"} · {employe?`${employe.prenom} ${employe.nom}`:"Employé"} · {note.date_frais}</p></div><span className="text-xs" style={{color:statut.couleur}}>{statut.libelle}</span><span className="font-mono font-semibold">{euros(note.montant_ttc)}</span></Link>})}{!(notesFrais??[]).length&&<p className="rounded border border-dashed p-4 text-sm text-neutral-500">Aucune note de frais n’est rattachée à ce chantier. Lors de la création d’une note, choisissez ce chantier dans le champ « Chantier ».</p>}</div>
        </section>}
