...
// Un montant inconnu s'affiche comme tel, jamais comme 0 €.
...
export default async function RentabilitePage() {
  ...
  // La garde de route exige déjà acces_rentabilite ; ce contrôle couvre aussi le rendu
  // du composant serveur lui-même : sans le droit, aucun chiffre n'est lu.
  if (!peutConsulterRentabilite(permissions)) {
    return <main className="p-8"><div className="mx-auto max-w-6xl space-y-4">
      <h1 className="text-xl font-semibold">Rentabilité des chantiers</h1>
      <p className="rounded-md border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800">{MESSAGE_RENTABILITE_REFUSEE}</p>
    </div></main>;
  }
  const supabase = await createClient();
  const peutUtiliserIA = iaEstActive() && aAccesIA(permissions);
  const [{ data: chantiers }, { data: factures }, { data: devis }, { data: donneesPointages }, { data: depenses }, { data: donneesIndemnites }, coutsStock, { data: donneesNotesFrais }, { data: couts }] = await Promise.all([
    ...
    // Le prix d'achat unitaire n'est plus lisible : total par chantier, ou refus explicite.
    lireCoutsStockChantiers(supabase, ctx.entrepriseId),
  ...
  const lignes = (chantiers ?? []).map((chantier) => {
    ...
    const coutStock = coutStockDuChantier(coutsStock, chantier.id);
    const coutNotesFrais = notesFrais.filter((item) => item.chantier_id === chantier.id).reduce((s, item) => s + Number(item.montant_ttc), 0);
    const { marge, taux } = margeChantier(factureHt, coutMainOeuvre + coutAchats + coutSousTraitance + coutIndemnitesPaie + coutNotesFrais, coutStock);
  ...
  const margesConnues = coutsStock.etat === "disponible";
  ...
  const totalCoutMo = lignes.reduce((s, ligne) => s + ligne.coutMainOeuvre, 0); const totalAchats = lignes.reduce((s, ligne) => s + ligne.coutAchats, 0); const totalSousTraitance = lignes.reduce((s, ligne) => s + ligne.coutSousTraitance, 0); const totalIndemnitesPaie = lignes.reduce((s, ligne) => s + ligne.coutIndemnitesPaie, 0); const totalNotesFrais = lignes.reduce((s, ligne) => s + ligne.coutNotesFrais, 0);
  const totalStock = totalCoutStock(coutsStock, lignes.map((ligne) => ligne.id));
  const totalMarge = totalStock === null ? null : totalFacture - (totalCoutMo + totalAchats + totalSousTraitance + totalIndemnitesPaie + totalStock + totalNotesFrais);
  const tauxGlobal = totalMarge === null ? null : totalFacture > 0 ? totalMarge / totalFacture * 100 : 0;
  const chantiersAvecActivite = lignes.filter((ligne) => ligne.factureHt > 0 || ligne.heures > 0 || ligne.budgetHt > 0);
  const meilleursChantiers = margesConnues ? [...chantiersAvecActivite].sort((a, b) => Number(b.marge) - Number(a.marge)).slice(0, 8) : [];
  const plusGrandVolume = Math.max(1, ...meilleursChantiers.map((ligne) => Math.max(ligne.factureHt, ligne.factureHt - Number(ligne.marge))));
  ...
  return <main className="p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-md border p-4 dark:border-neutral-800">
      ...
      <div className="space-y-3">{!margesConnues ? <p className="py-8 text-center text-sm text-neutral-500">Classement indisponible : le coût du stock n’est pas connu, les marges ne sont pas calculées.</p> : meilleursChantiers.length ? meilleursChantiers.map((ligne) => { const marge = Number(ligne.marge); const cout = ligne.factureHt-marge; return <Link key={ligne.id} href={`/chantiers/${ligne.id}`} className="grid gap-2 rounded-md p-2 hover:bg-neutral-50 dark:hover:bg-neutral-900 sm:grid-cols-[180px_1fr_110px] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-medium">{ligne.nom}</p><p className="truncate text-xs text-neutral-500">{ligne.clientNom}</p></div><div className="space-y-1"><div className="h-2 rounded-full bg-neutral-100 dark:bg-neutral-800"><div className="h-2 rounded-full bg-blue-500" style={{width:`${ligne.factureHt/plusGrandVolume*100}%`}}/></div><div className="h-2 rounded-full bg-neutral-100 dark:bg-neutral-800"><div className="h-2 rounded-full bg-orange-400" style={{width:`${Math.max(0,cout)/plusGrandVolume*100}%`}}/></div></div><div className={`text-right text-sm font-semibold ${marge>=0?"text-green-700":"text-red-700"}`}>{euros(marge)}<p className="text-xs font-normal text-neutral-500">{ligne.taux===null?"non facturé":`${ligne.taux.toFixed(1)} %`}</p></div></Link> }) : <p className="py-8 text-center text-sm text-neutral-500">Aucune activité chiffrée à afficher.</p>}</div>
    </section>
    <div>
      ...
      <div className="overflow-x-auto rounded-md border dark:border-neutral-800"><table className="w-full min-w-[1380px] table-fixed text-sm"><thead className="sticky top-0 bg-neutral-100 text-left text-xs uppercase text-neutral-600 dark:bg-neutral-900 dark:text-neutral-300"><tr><th className="w-52 px-3 py-3">Chantier</th><th className="w-40 px-3">Client</th><th className="w-28 px-3 text-right">Facturé HT</th><th className="w-20 px-3 text-right">Heures</th><th className="w-28 px-3 text-right">Coût MO</th><th className="w-28 px-3 text-right">Achats</th><th className="w-28 px-3 text-right">Stock</th><th className="w-32 px-3 text-right">Sous-traitance</th><th className="w-28 px-3 text-right">Frais</th><th className="w-28 px-3 text-right">Indemnités</th><th className="w-28 px-3 text-right">Marge</th><th className="w-24 px-3 text-right">Taux</th></tr></thead><tbody>{lignes.map((ligne,index)=><tr key={ligne.id} className={`border-t dark:border-neutral-800 ${index%2?"bg-neutral-50/60 dark:bg-neutral-900/40":""}`}><td className="px-3 py-3"><Link href={`/chantiers/${ligne.id}`} className="font-medium hover:underline">{ligne.nom}</Link><div className="font-mono text-xs text-neutral-400">{ligne.reference_interne}</div></td><td className="truncate px-3" title={ligne.clientNom}>{ligne.clientNom}</td><td className="px-3 text-right font-mono">{euros(ligne.factureHt)}</td><td className="px-3 text-right font-mono">{ligne.heures} h</td><td className="px-3 text-right font-mono">{euros(ligne.coutMainOeuvre)}{ligne.coutHoraireManquant && <span title="Coût horaire manquant pour au moins un salarié : coût sous-estimé" className="ml-1 text-amber-600">⚠</span>}</td><td className="px-3 text-right font-mono">{euros(ligne.coutAchats)}</td><td className="px-3 text-right font-mono">{montant(ligne.coutStock)}</td><td className="px-3 text-right font-mono">{euros(ligne.coutSousTraitance)}</td><td className="px-3 text-right font-mono">{euros(ligne.coutNotesFrais)}</td><td className="px-3 text-right font-mono">{euros(ligne.coutIndemnitesPaie)}</td><td className={`px-3 text-right font-mono font-semibold ${ligne.marge===null?"text-neutral-500":ligne.marge>=0?"bg-green-50 text-green-700 dark:bg-green-950/30":"bg-red-50 text-red-700 dark:bg-red-950/30"}`}>{montant(ligne.marge)}</td><td className="px-3 text-right"><span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${ligne.taux===null?"bg-neutral-100 text-neutral-500":ligne.taux>=20?"bg-green-100 text-green-800":ligne.taux>=0?"bg-amber-100 text-amber-800":"bg-red-100 text-red-800"}`}>{ligne.marge===null?"Indisponible":ligne.taux===null?"—":`${ligne.taux.toFixed(1)} %`}</span></td></tr>)}</tbody></table></div>
    </div>
  </div></main>;
}
