...
export default async function RentabilitePage() {
  ...
  return <main className="p-8"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="overflow-x-auto rounded-md border"><table className="w-full min-w-[1100px] text-sm"><thead className="bg-neutral-50 text-left text-xs uppercase text-neutral-500"><tr><th className="px-3 py-2">Chantier</th><th>Client</th><th className="text-right">Facturé HT</th><th className="text-right">Heures</th><th className="text-right">Coût MO</th><th className="text-right">Achats</th><th className="text-right">Stock</th><th className="text-right">Sous-traitance</th><th className="text-right">Notes de frais</th><th className="text-right">Indemnités paie</th><th className="text-right">Marge</th><th className="px-3 text-right">Taux</th></tr></thead><tbody>{lignes.map(ligne=><tr key={ligne.id} className="border-t"><td className="px-3 py-2"><Link href={`/chantiers/${ligne.id}`} className="font-medium hover:underline">{ligne.nom}</Link><div className="font-mono text-xs text-neutral-400">{ligne.reference_interne}</div></td><td>{ligne.clientNom}</td><td className="text-right">{euros(ligne.factureHt)}</td><td className="text-right">{ligne.heures} h</td><td className="text-right">{euros(ligne.coutMainOeuvre)}{ligne.coutHoraireManquant && <span title="Coût horaire manquant pour au moins un salarié : coût sous-estimé" className="ml-1 text-amber-600">⚠</span>}</td><td className="text-right">{euros(ligne.coutAchats)}</td><td className="text-right">{euros(ligne.coutStock)}</td><td className="text-right">{euros(ligne.coutSousTraitance)}</td><td className="text-right">{euros(ligne.coutNotesFrais)}</td><td className="text-right">{euros(ligne.coutIndemnitesPaie)}</td><td className={`text-right font-medium ${ligne.marge>=0?"text-green-700":"text-red-700"}`}>{euros(ligne.marge)}</td><td className="px-3 text-right">{ligne.taux===null?"—":`${ligne.taux.toFixed(1)} %`}</td></tr>)}</tbody></table></div>
  </div></main>;
}
