...
export default async function DepensePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  return (
    <main className="p-4 sm:p-8"><div className="mx-auto max-w-4xl space-y-6">
      ...
      {peutGerer && reste > 0 && !["annulee", "litige"].includes(depense.statut) && <form action={ajouterReglementDepenseAction.bind(null, id)} className="grid gap-2 rounded border p-4 dark:border-neutral-800 sm:grid-cols-2 lg:grid-cols-4"><div className="sm:col-span-2 lg:col-span-4"><h2 className="font-semibold">Enregistrer un paiement fournisseur effectué</h2><p className="text-xs text-neutral-500">À utiliser après le virement, prélèvement, paiement par carte ou chèque depuis le compte de l’entreprise.</p>{peutPreparerVirement && <Link href="/paiements-bancaires" className="mt-2 inline-flex text-xs font-semibold text-blue-700 hover:underline">Préparer et transmettre ce virement depuis Banque & paie →</Link>}</div><input name="montant" type="number" min="0.01" max={reste} step="0.01" defaultValue={reste} required className="rounded border px-3 py-2 text-sm dark:bg-neutral-900"/><input name="date" type="date" defaultValue={new Date().toISOString().slice(0,10)} required className="rounded border px-3 py-2 text-sm dark:bg-neutral-900"/><select name="mode" className="rounded border px-3 py-2 text-sm dark:bg-neutral-900"><option value="virement">Virement</option><option value="prelevement">Prélèvement</option><option value="cb">Carte</option><option value="cheque">Chèque</option><option value="especes">Espèces</option></select><input name="reference" placeholder="Référence bancaire" className="rounded border px-3 py-2 text-sm dark:bg-neutral-900"/><button className="rounded bg-neutral-900 px-4 py-2 text-sm text-white dark:bg-white dark:text-neutral-900 sm:col-span-2 lg:col-span-4">Marquer ce paiement fournisseur comme effectué</button></form>}

      <section><h2 className="mb-2 font-semibold">Paiements fournisseur — sorties d’argent</h2><p className="mb-3 rounded-md bg-amber-50 p-3 text-xs text-amber-900">Enregistrez ici un paiement réellement effectué depuis la banque, par prélèvement, carte ou chèque. Liria ne déclenche aucun virement bancaire fournisseur tant qu’un prestataire bancaire réglementé n’est pas connecté.</p>{reglements?.map((reglement) => <div key={reglement.id} className="flex flex-wrap items-center justify-between gap-3 border-t py-2 text-sm"><span>{reglement.date} · {reglement.mode}</span><span className="text-neutral-500">{reglement.reference ?? ""}</span><strong>{euros(reglement.montant)}</strong>{peutGerer && <form action={supprimerReglementDepenseAction.bind(null, id, reglement.id)}><ConfirmSubmitButton message={`Annuler ce règlement de ${euros(reglement.montant)} ? Le solde de la facture fournisseur sera recalculé automatiquement.`} className="text-xs text-red-600 hover:underline">Annuler ce paiement</ConfirmSubmitButton></form>}</div>)}{(!reglements || !reglements.length) && <p className="text-sm text-neutral-500">Aucun paiement fournisseur enregistré.</p>}</section>
    </div></main>
  );
}
