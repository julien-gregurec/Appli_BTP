...
import { calculerRepartitionComptesFacturables, TARIFS_COMPTES_SUPPLEMENTAIRES, TYPES_COMPTES_TARIFAIRES } from "@/lib/facturation-comptes";
...
export default async function AbonnementPage({ searchParams }: { searchParams: Promise<{ error?: string; succes?: string }> }) {
  ...
  const [{ data: entreprise }, { data: utilisationStockage }, { data: employesFacturables }, { data: postes }, { data: appareils }, consommationIA, { data: facturesAbonnement }, { data: historique }] = await Promise.all([
    ...
    supabase.from("postes").select("id,nom,code_offre,tarif_compte_mensuel").eq("entreprise_id", ctx.entrepriseId),
  ...
  const repartitionComptes = calculerRepartitionComptesFacturables({ employes: employesFacturables ?? [], postes: postes ?? [] });
  const supplementAppareilsMensuel = depassementsAppareils.reduce((total, ligne) => total + ligne.supplementMensuelHt, 0);
  const prixComptes = prixAbonnementMensuel(repartitionComptes, offre);
  const nbComptesFacturables = prixComptes.detailComptes.totalComptes;
  ...
  const abonnementAvantRemiseMensuel = prixComptes.total === null ? null : prixComptes.total + supplementAppareilsMensuel;
  const coutPeriodeEstime = abonnementAvantRemiseMensuel === null || prixComptes.totalAnnuel === null
    ? null
    : annuel
      ? prixComptes.totalAnnuel + supplementAppareilsMensuel * MOIS_FACTURES_EN_ANNUEL + stockageMensuel.montantHt * MOIS_FACTURES_EN_ANNUEL
      : abonnementAvantRemiseMensuel + stockageMensuel.montantHt;
  const coutMensuelEstime = coutPeriodeEstime === null ? null : annuel ? coutPeriodeEstime / 12 : coutPeriodeEstime;
    ...
    <section className="rounded-xl border p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="text-right">
          <p className="text-2xl font-bold">{coutPeriodeEstime === null ? "Selon contrat" : euros(coutPeriodeEstime)} {coutPeriodeEstime !== null&&<span className="text-sm font-normal text-neutral-500">HT/{annuel ? "an" : "mois"}</span>}</p>
          {annuel&&coutMensuelEstime!==null&&<p className="text-xs text-neutral-500">soit {euros(coutMensuelEstime)} HT/mois en moyenne</p>}
        </div>
      </div>
      <dl className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg bg-neutral-50 p-3 dark:bg-neutral-900"><dt className="text-xs uppercase text-neutral-500">Offre {offre.nom}</dt><dd className="mt-1 font-semibold">{prixComptes.base === null ? "Sur devis" : `${euros(prixComptes.base)} HT/mois`}</dd><p className="text-xs text-neutral-500">{offre.devisObligatoire ? "Capacité définie au contrat" : offre.cle === "entreprise" ? "40 salariés + 10 administrateurs inclus" : `${offre.comptesInclus} compte(s) inclus`}</p></div>
        <div className="rounded-lg bg-neutral-50 p-3 dark:bg-neutral-900"><dt className="text-xs uppercase text-neutral-500">Comptes de l’entreprise</dt><dd className="mt-1 font-semibold">{nbComptesFacturables} compte(s) facturable(s)</dd>{prixComptes.employesSupplementaires > 0 ? <ul className="mt-1 space-y-0.5 text-xs text-neutral-500">{TYPES_COMPTES_TARIFAIRES.map((type) => prixComptes.detailComptes.supplementaires[type] > 0 ? <li key={type}>{prixComptes.detailComptes.supplementaires[type]} {TARIFS_COMPTES_SUPPLEMENTAIRES[type].libelle.toLowerCase()} × {euros(TARIFS_COMPTES_SUPPLEMENTAIRES[type].prixMensuelHt)} HT/mois</li> : null)}</ul> : <p className="text-xs text-neutral-500">Aucun compte supplémentaire</p>}</div>
