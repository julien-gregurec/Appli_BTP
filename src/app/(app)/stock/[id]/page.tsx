...
export default async function ArticleStockPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ data: article }, { data: teintes }, { data: mouvements }, { data: code }, { data: fichesTechniques }] = await Promise.all([
  ...
  return <main className="p-3 sm:p-8"><div className="mx-auto max-w-4xl space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-4"><div className="rounded border p-4"><p className="text-xs text-neutral-500">Stock</p><strong className="text-xl">{article.quantite_stock} {article.unite}</strong></div><div className="rounded border p-4"><p className="text-xs text-neutral-500">Seuil</p><strong className="text-xl">{article.seuil_alerte} {article.unite}</strong></div><div className="rounded border p-4"><p className="text-xs text-neutral-500">Prix achat HT</p><strong className="text-xl">{euros(article.prix_achat_ht)}</strong></div><div className="rounded border p-4"><p className="text-xs text-neutral-500">Prix revente HT</p><strong className="text-xl">{euros(article.prix_vente_ht)}</strong></div></div>
    <form action={modifierPrixArticleStockAction.bind(null,id)} className="grid gap-3 rounded border p-4 sm:grid-cols-[1fr_1fr_auto]"><label className="text-xs text-neutral-500">Prix d’achat HT<input name="prix_achat_ht" type="number" min="0" step="0.01" required defaultValue={article.prix_achat_ht} className="mt-1 w-full rounded border px-3 py-2 text-sm"/></label><label className="text-xs text-neutral-500">Prix de revente HT<input name="prix_vente_ht" type="number" min="0" step="0.01" required defaultValue={article.prix_vente_ht} className="mt-1 w-full rounded border px-3 py-2 text-sm"/></label><button className="self-end rounded bg-[#0d1b2a] px-4 py-2 text-sm font-semibold text-white">Enregistrer les prix</button></form>
