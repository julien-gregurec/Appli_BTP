...
export default async function FacturationAvanceePage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ data: devis }, { data: situations }, { data: remises }] = await Promise.all([
    supabase.from("devis").select("id,numero,montant_ht,chantier_id,client:clients(nom,prenom,societe),chantier:chantiers(nom)").eq("entreprise_id", ctx.entrepriseId).eq("statut", "accepte").order("date_emission", { ascending: false }),
  ...
  return <main className="p-4 sm:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <form action={creerSituationAction} className="space-y-3 rounded-xl border p-4"><div><h2 className="font-semibold">Nouvelle situation de travaux</h2><p className="text-xs text-neutral-500">L’avancement indiqué est le cumul total du marché.</p></div>
        <label className="block text-xs">Devis accepté<select required name="devis_id" className={champ}><option value="">Choisir…</option>{(devis ?? []).filter(d => d.chantier_id).map(d => { const c=un(d.client); const ch=un(d.chantier); return <option key={d.id} value={d.id}>{d.numero} · {libelleClient(c)} · {ch?.nom} · {euros(d.montant_ht)}</option>; })}</select></label>
        ...
        <label className="block text-xs">Notes<textarea name="notes" rows={2} className={champ}/></label><button className="w-full rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white">Calculer la situation</button>
      </form>
      <form action={creerFactureAvanceeAction} className="space-y-3 rounded-xl border p-4"><div><h2 className="font-semibold">Acompte, avoir ou solde</h2><p className="text-xs text-neutral-500">Le document est créé en brouillon avant émission.</p></div>
        <label className="block text-xs">Devis accepté<select required name="devis_id" className={champ}><option value="">Choisir…</option>{(devis ?? []).map(d => { const ch=un(d.chantier); return <option key={d.id} value={d.id}>{d.numero} · {libelleClient(un(d.client))} · {ch?.nom ?? "Sans chantier"} · {euros(d.montant_ht)}</option>; })}</select></label>
        ...
        <label className="flex items-center gap-2 rounded-md bg-neutral-50 p-3 text-sm dark:bg-neutral-900"><input type="checkbox" name="est_dgd" value="true"/> Marquer comme décompte général définitif (DGD)</label><button className="w-full rounded-md bg-neutral-900 px-4 py-2 text-sm font-semibold text-white dark:bg-white dark:text-neutral-900">Créer le brouillon</button>
      </form>
    </div>
