...
export default async function DevisDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const { data: devis } = await supabase
    .from("devis")
    .select("*, client:clients!devis_client_id_fkey(id, nom, prenom, societe, email), chantier:chantiers!devis_chantier_id_fkey(id, nom)")
    .eq("id", id)
  ...
  return (
    <main className="p-8">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {peutGererDevis && <section className="rounded-md border border-blue-200 bg-blue-50/40 p-4 dark:border-blue-900 dark:bg-blue-950/20"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Chantier associé au devis</h2><p className="text-sm text-neutral-500">La liste contient uniquement les chantiers du même client. La fiche chantier affichera automatiquement ce devis.</p></div>{chantier && <Link href={`/chantiers/${chantier.id}`} className="text-sm font-medium text-blue-700 hover:underline dark:text-blue-300">Ouvrir {chantier.nom}</Link>}</div>{devis.statut === "accepte" ? (<p className="mt-2 text-xs text-amber-800 dark:text-amber-300">Ce devis est accepté : son chantier ne peut plus être modifié. Pour le rattacher à un autre chantier, dupliquez-le.</p>) : (<form action={associerDevisChantierAction.bind(null, id, `/devis/${id}`)} className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end"><label className="flex-1 text-xs text-neutral-500">Sélectionner un chantier<SearchableSelect name="chantier_id" defaultValue={devis.chantier_id ?? ""} options={chantiersClient.map((item) => ({ value: item.id, label: `${item.nom}${item.ville ? ` · ${item.ville}` : ""}`, search: item.statut }))} placeholder="Écrire le nom du chantier…" emptyLabel="— Aucun chantier —" className="mt-1" /></label><button className="rounded-md bg-[#0d1b2a] px-4 py-2 text-sm font-semibold text-white">Enregistrer l’association</button></form>)}</section>}
