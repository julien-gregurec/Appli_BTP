...
export default async function PointagePage({searchParams}:{searchParams:Promise<{mois?:string;error?:string;succes?:string}>}){
  ...
  return <main className="ecran-mobile p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section><h2 className="mb-3 font-semibold">Mes pointages GPS</h2><div className="space-y-3">{sessions.map(s=>{const c=un(s.chantier),validation=un(s.pointage),duree=s.depart_at?Math.max(0,(new Date(s.depart_at).getTime()-new Date(s.arrivee_at).getTime())/3600000-Number(s.pause_minutes)/60):null;return <article key={s.id} className={`rounded-md border p-4 ${validation?.anomalie_niveau==="critique"?"border-red-400 bg-red-50":validation?.anomalie_niveau?"border-amber-400 bg-amber-50":""}`}><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-semibold text-[#9a7625]">{c?.nom??"Chantier non renseigné"}</h3><p className="mt-1 text-sm text-neutral-500">{dateHeure(s.arrivee_at)} · arrivée {heure(s.arrivee_at)}{s.depart_at?` · départ ${heure(s.depart_at)}`:" · départ non pointé"}</p>{s.tache&&<p className="mt-1 text-sm">{s.tache}</p>}{validation?.anomalie_motif&&<p className="mt-2 rounded bg-white/70 p-2 text-sm font-medium text-amber-900">⚠ {validation.anomalie_motif}{validation.heures_attendues!==null?` · ${validation.heures_attendues} h attendues`:""}</p>}</div>{duree!==null&&<strong className="rounded bg-neutral-100 px-3 py-1 font-mono dark:bg-neutral-800">{duree.toFixed(2).replace(".",",")} h</strong>}</div><div className="mt-3 flex flex-wrap gap-4 text-xs">{s.latitude_arrivee!==null&&s.longitude_arrivee!==null?<a href={`https://www.openstreetmap.org/?mlat=${s.latitude_arrivee}&mlon=${s.longitude_arrivee}#map=18/${s.latitude_arrivee}/${s.longitude_arrivee}`} target="_blank" rel="noreferrer" className="font-medium text-blue-700 hover:underline">GPS arrivée{s.precision_arrivee_metres?` · ± ${Math.round(Number(s.precision_arrivee_metres))} m`:""}</a>:<span className="text-neutral-500">Pas de position à l’arrivée</span>}{s.latitude_depart!==null&&s.longitude_depart!==null&&<a href={`https://www.openstreetmap.org/?mlat=${s.latitude_depart}&mlon=${s.longitude_depart}#map=18/${s.latitude_depart}/${s.longitude_depart}`} target="_blank" rel="noreferrer" className="font-medium text-blue-700 hover:underline">GPS départ{s.precision_depart_metres?` · ± ${Math.round(Number(s.precision_depart_metres))} m`:""}</a>}</div></article>;})}{!sessions.length&&<p className="rounded border border-dashed p-6 text-center text-sm text-neutral-500">Aucun pointage GPS pour ce mois.</p>}</div></section>
