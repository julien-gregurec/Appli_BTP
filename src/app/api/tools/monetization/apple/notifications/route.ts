...
export async function POST(request: Request) {
  ...
  try {
    const { notification, payload, environment } = await verifyAppleNotification(body.signedPayload);
    if (!notification.notificationUUID) throw new Error("Identifiant absent");
    /* L'environnement vient de la notification VÉRIFIÉE, jamais du déploiement. */
    const reservation = await reserveToolsMonetizationEvent({ provider: "apple", environment, externalEventId: notification.notificationUUID,
    ...
    if (payload) {
      ...
      await admin.from("tools_monetization_events").update({ user_id: payload.user_id, subscription_id: subscriptionId, status: "processed", processed_at: new Date().toISOString(), after_state: { status: payload.status } })
        .eq("provider", "apple").eq("environment", environment).eq("external_event_id", notification.notificationUUID);
