...
export async function POST(request: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        for await (const evenement of demanderAssistantIAStream(supabase, ctx.entrepriseId, ctx.entrepriseNom, ctx.userId, ctx.prenom, peutGererPlanning, permissions, historique)) {
          envoyer(evenement);
        }
        envoyer({ type: "fin" });
