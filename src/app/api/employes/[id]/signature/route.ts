...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!employe?.signature_storage_path) return NextResponse.json({ error: "Signature introuvable" }, { status: 404 });
  const { data, error } = await supabase.storage.from("documents-employes").createSignedUrl(employe.signature_storage_path, 120);
