      ...
      <div className={`mx-auto overflow-hidden rounded-lg border bg-white shadow-xl transition-[max-width] dark:bg-neutral-950 ${mobile ? "max-w-[390px]" : "max-w-[1050px]"}`}>
        ...
        <div className={`flex ${mobile ? "min-h-[650px]" : "min-h-[620px]"}`}>
          {!mobile && <aside className="w-52 shrink-0 bg-[#0d1b2a] p-3 text-white">
            ...
            <nav className="mt-3 space-y-0.5">{navigation.map((item) => <div key={item.href} className={`rounded px-2.5 py-1.5 text-xs ${item.href === "/dashboard" ? "bg-[#c9a24a] font-medium text-[#0d1b2a]" : "text-white/75"}`}>{item.label}</div>)}</nav>
          </aside>}
          <div className="min-w-0 flex-1 p-4 sm:p-6">
            ...
            {mobile && <div className="mt-4 rounded-md border p-3"><div className="text-xs font-semibold">Menu visible</div><div className="mt-2 flex flex-wrap gap-1.5">{navigation.map((item) => <span key={item.href} className="rounded bg-neutral-100 px-2 py-1 text-[10px] dark:bg-neutral-800">{item.label}</span>)}</div></div>}
