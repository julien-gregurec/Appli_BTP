...
export function PointageArriveeDepart({ employes, chantiers, sessions }: { employes: Option[]; chantiers: Option[]; sessions: Session[] }) {
  return <section className="space-y-4 rounded-lg border-2 border-[#c9a24a]/60 bg-[#c9a24a]/5 p-5">
    ...
    <form action={enregistrerArriveeAction} encType="multipart/form-data" className="space-y-3 rounded-md bg-white p-4 shadow-sm dark:bg-neutral-950">
    ...
    <div className="grid gap-3">{sessions.map((session) => <form key={session.id} action={enregistrerDepartAction.bind(null, session.id)} encType="multipart/form-data" className="space-y-3 rounded-md border border-blue-200 bg-white p-4 dark:bg-neutral-950">
