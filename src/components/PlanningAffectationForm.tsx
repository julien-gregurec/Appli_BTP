...
export function PlanningAffectationForm({ action, retour, debut, fin, chantiers, employes }: { action: (formData: FormData) => void | Promise<void>; retour: string; debut: string; fin: string; chantiers: Option[]; employes: Employe[] }) {
  ...
  return (
    <form action={action} className="grid gap-3 rounded-md border p-4 dark:border-neutral-800 lg:grid-cols-[180px_1.4fr_160px_110px_1.4fr]">
      ...
      {chantier ? <label className="text-xs text-neutral-500">Chantier<select name="chantier_id" required className={input}><option value="">Choisir un chantier</option>{chantiers.map((item) => <option key={item.id} value={item.id}>{item.nom}</option>)}</select></label> : <label className="text-xs text-neutral-500">Lieu / précision<input name="lieu_activite" placeholder={type === "depot" ? "Dépôt principal" : type === "visite_medicale" ? "Centre de médecine du travail" : "Facultatif"} className={input} /></label>}
