...
// sur ces tables reste large (accès par simple appartenance à l'entreprise), l'application
// du droit fin se fait normalement au niveau page/action. Le copilote doit reproduire
// cette même restriction explicitement, sinon il devient un contournement en langage
// naturel des droits de menu (ex. un poste Terrain sans acces_rentabilite ne doit jamais
// obtenir la marge d'un chantier via l'IA). heures_supplementaires_semaine agrège les
// heures de toute l'équipe (pas seulement l'utilisateur courant) : seul gerer_pointage
// (contrôle d'équipe, cf. séparation pointage personnel/contrôle équipe) y donne droit,
// pas saisir_son_pointage.
const PERMISSION_REQUISE_OUTIL: Partial<Record<string, readonly string[]>> = {
  ...
  heures_supplementaires_semaine: ["gerer_pointage"],
};
...
export async function executerOutilCopilote(
  ...
  // Deuxième barrière en profondeur : outilsAutorisesCopilote() retire déjà ces outils
  // de la liste proposée au modèle, mais on ne fait jamais confiance uniquement à ce que
