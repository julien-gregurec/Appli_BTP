 ...
 *
 * `getContexteEntreprise()` redirige vers /login quand la session manque. C'est le bon
 * comportement pour un écran, et le mauvais pour une route d'API : un client hors-ligne qui
 * reçoit un 307 vers une page HTML ne peut pas le distinguer d'une réponse métier. Il
 * enregistrerait « envoyé » une mutation jamais reçue.
 *
 * ── Trois issues, et non deux ───────────────────────────────────────────────────────────
 *
 * Confondre « pas de session » et « service d'authentification injoignable » ferait annoncer
 * « session expirée » à un utilisateur parfaitement connecté dont le serveur a simplement mis
 * trop de temps à répondre. Il se reconnecterait pour rien, et sa file resterait bloquée en
 * échec alors qu'un simple réessai suffisait.
 *
 * Le principe est repris de Réserves, dont le moteur hors-ligne a rencontré exactement ce
 * défaut. Le code, lui, est propre à Gestion Pro.
 */
...
export async function resoudreIdentite(): Promise<ResolutionIdentite> {
  ...
  // L'entreprise active est exigée : une mutation appartient toujours à un locataire.
