...
export function reponseCsv(lignes: unknown[][], nomFichier: string) { return reponseTexteCsv(csv(lignes), nomFichier); }
/** Même réponse que `reponseCsv`, pour un texte déjà produit par `csv()`. `nomFichier` doit être sûr (ASCII, sans guillemet). */
export function reponseTexteCsv(texte: string, nomFichier: string) { return new Response(texte, { headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="${nomFichier}"`, "Cache-Control": "private, no-store" } }); }
