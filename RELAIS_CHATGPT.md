...
[IDENTIFIANTS HISTORIQUES RETIRÉS — rotation requise]

Le retrait du fichier courant ne supprime pas ces identifiants de l'historique Git. Leur révocation ou rotation manuelle reste obligatoire, sans réécriture d'historique dans cette mission.
...
- Incident d’encodage constaté après collage du seed dans le SQL Editor : certains accents des seules données JUJU ont été enregistrés sous forme `√©`/`Ã`. Le correctif ciblé `supabase/production/corriger_encodage_juju.sql` est prêt, ASCII uniquement, et placé dans le presse-papiers ; il reste à l’exécuter dans Supabase. Le seed source utilise désormais des échappements Unicode pour ses valeurs afin d’éviter une récidive.
...
- Le menu principal ordinateur/mobile contient aussi un bouton permanent « Guide d’utilisation · PDF » pour les comptes ordinaires. Le compte dépôt reste volontairement limité à Stock/Borne/Dépôt et ne reçoit pas ce raccourci.
...
## 100. Collaboration, appels d’offres, DOE et reprise de données — 17 juillet 2026

- Migration `20260717000096_collaboration_appels_offres_doe.sql` appliquée dans Supabase et contrôlée : messagerie, appels d’offres, fiches techniques, générations DOE, e-mails chantier, écritures comptables importées et permission de messagerie sont présents.
- Les notes de frais sont regroupées par salarié avant validation. Un responsable ouvre une ligne salarié pour voir les demandes, leurs statuts et leurs montants. La fiche de chaque employé contient maintenant son historique de notes de frais, dans le respect des droits personnels et d’équipe existants.
- Nouveau module `/messagerie` : conversation privée avec un collaborateur ou fil partagé lié à un chantier. L’annuaire utilisé par la messagerie ne retourne que nom, prénom et poste ; aucun coût salarial ni donnée RH sensible n’est exposé.
- Nouveau module `/appels-offres` : référence, client, chantier, donneur d’ordre, dates, budget, statut, lien vers le dossier et notes. Les droits de consultation et de gestion sont séparés.
- Nouveau DOE automatique par chantier : l’écran `/chantiers/[id]/doe` rassemble les pièces du chantier, plans, produits réellement sortis du stock, fiches techniques et fiches de sécurité. Chaque génération fige un manifeste versionné et signale les documents manquants. L’impression navigateur produit le PDF sans prétendre à une conformité juridique non validée.
- Chaque article de stock possède maintenant un dossier technique privé. PDF et images peuvent être classés comme fiche technique, fiche de sécurité, notice de pose ou certificat, puis consultés par URL signée temporaire. Les documents sont automatiquement repris dans le DOE quand le produit a été utilisé sur le chantier.
- Les e-mails peuvent être archivés manuellement dans un chantier avec expéditeur, destinataires, objet, résumé, date et sens entrant/sortant. La page Connecteurs prépare Google, Microsoft ou IMAP sans stocker de mot de passe. La synchronisation automatique réelle reste volontairement inactive tant que les identifiants OAuth, les autorisations du fournisseur et les webhooks ne sont pas configurés.
- L’import entreprise pilote accepte désormais les articles de stock et codes-barres ainsi que les écritures comptables en CSV/XLSX, en plus des données déjà supportées. Les articles sont rapprochés par référence ; les écritures exigent une date, un journal, un compte et des débits/crédits valides. Le transfert direct depuis Batappli reste dépendant d’un export officiel ou d’une API fournie par l’éditeur.
